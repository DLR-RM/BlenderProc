...
def write_bop(
    ...
    if calc_mask_info_coco:
        ...
        for obj in dataset_objects:
            ...
            if not np.all(
                np.isclose(np.array(obj.blender_obj.scale), obj.blender_obj.scale[0])
            ):
                print(
...
class _BopWriterUtility:
    ...
    def write_frames(
        ...
        :param frames_per_chunk: Number of frames saved in each chunk (called scene in BOP)
        """
        if not depths and not colors:
            print("No depth and color information in write_frames. Returning...")
            return

        # Format of the depth and edge images.
        depth_ext = ".png"
        ...
        if len(depths) != len(colors) != num_new_frames:
            raise Exception(
                "The amount of images stored in the depths/colors does not correspond "
                "to the amount of images specified by frame_start to frame_end."
            )
        ...
        for frame_id in range(
            ...
            if curr_frame_id == 0:
                ...
                os.makedirs(
                    os.path.dirname(depth_tpath.format(chunk_id=curr_chunk_id, im_id=0))
                )
            ...
            if color_file_format == "PNG":
                ...
                cv2.imwrite(
                    rgb_fpath, color_bgr, [int(cv2.IMWRITE_JPEG_QUALITY), jpg_quality]
                )
            ...
            if ((curr_frame_id + 1) % frames_per_chunk == 0) or (
                ...
                # Save camera info.
                _BopWriterUtility.save_json(
                    chunk_camera_tpath.format(chunk_id=curr_chunk_id), chunk_camera
                )
    ...
    @staticmethod
    def _pyrender_init(
        ren_width: int, ren_height: int, trimesh_objects: Dict[int, trimesh.Trimesh]
    ):
    ...
    def calc_gt_masks(
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            for im_counter, im_id in enumerate(im_ids):
                ...
                # Load depth image.
                depth_path = os.path.join(chunk_dir, "depth", "{im_id:06d}.png").format(
                    im_id=im_id
                )
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            if dir_counter == 0 and starting_frame_id > 0:
                ...
                scene_gt_info = _BopWriterUtility.load_json(
                    os.path.join(chunk_dir, "scene_gt_info.json"), keys_to_int=True
                )
            else:
                scene_gt_info = {}
    ...
    def calc_gt_coco(
        ...
        for dir_counter, chunk_dir in enumerate(chunk_dirs):
            ...
            for scene_view, inst_list in scene_gt.items():
                ...
                img_path = os.path.join(chunk_dir, "rgb", "{im_id:06d}.jpg").format(
                    im_id=im_id
                )
