...
def random_walk(total_length: int, dims: int, step_magnitude: float = 1.0, window_size: int = 1, 
    ...
    :param window_size: Convolve the final trajectory with an average filter that smoothens the trajectory with a given filter size.
    :param interval: Constrain the random walk to an interval and mirror steps if they go beyond. List of arrays with dimension dims.
    ...
    if distribution == 'uniform':
        ...
        raise('Unknown distribution: ', distribution, 'Choose between "normal" and "uniform"') 
    
    # Sample random steps
    random_steps = step_magnitude * np.random.choice([-1,1], (total_length, dims)) * dist_fun(total_length, dims)**order
    ...
    if window_size > 1:
        ...
        for i in range(dims):
            cumulative_steps[:,i] = np.convolve(cumulative_steps_padded[:,i], np.ones(window_size) / window_size, 'valid') 
