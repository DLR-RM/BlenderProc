...
from blenderproc.main.GlobalStorage import GlobalStorage
from blenderproc.utility.Config import Config
import numpy as np
import json
...
class Utility:
                ...
                for i in range(amount_of_repetitions):
                    ...
                    for suffix in ["Module", ""]:
                        try:
                            ...
                            module = importlib.import_module("blenderproc." + module_config["module"] + suffix)
                    ...
                    if module_class is None:
                        raise Exception("The module blenderproc." + module_config["module"] + " was not found!")
    ...
    class UndoAfterExecution:
        ...
        def __enter__(self):
            if self._perform_undo_op:
                from blenderproc.utility.StructUtility import Struct
    ...
    def build_provider(name, parameters):
        ...
        for suffix in ["Module", ""]:
            try:
                # Import class from blenderproc.utility
                module_class = getattr(importlib.import_module("blenderproc.provider." + name + suffix), name.split(".")[-1] + suffix)
                break
            except ModuleNotFoundError:
                # Try next suffix
                continue
        ...
        # Throw an error if no module/class with the specified name + any suffix has been found
        if module_class is None:
            raise Exception("The module blenderproc.provider." + name + " was not found!")
