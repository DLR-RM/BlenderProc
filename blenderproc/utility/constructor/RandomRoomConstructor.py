...
from blenderproc.provider.getter.Material import Material
from blenderproc.utility.BlenderUtility import get_bound_volume, check_bb_intersection_on_values
from blenderproc.utility.CollisionUtility import CollisionUtility
from blenderproc.utility.EntityUtility import Entity
from blenderproc.utility.MeshObjectUtility import MeshObject
from blenderproc.utility.Utility import Utility, Config
from blenderproc.utility.object.FloorExtractor import FloorExtractor


class RandomRoomConstructor:

    @staticmethod
    def construct_random_room(used_floor_area: float, amount_of_extrusions: int, fac_from_square_room: float, corridor_width: float, wall_height: float, amount_of_floor_cuts: int, only_use_big_edges: bool, create_ceiling: bool) -> Tuple[MeshObject, MeshObject, MeshObject]:
        """
        ...
        def extract_plane_from_room(obj: MeshObject, used_split_height: float, up_vec: mathutils.Vector,
            ...
            counter = FloorExtractor.select_at_height_value(bm, used_split_height, compare_height,
                                                           mathutils.Vector(up_vec), compare_angle, obj.get_local2world_mat())
    ...
    @staticmethod
    def assign_materials_to_floor_wall_ceiling(floor_obj: MeshObject, wall_obj: MeshObject, ceiling_obj: MeshObject, assign_material_to_ceiling: bool, materials: [Material]):
        """
    ...
    @staticmethod
    def sample_new_object_poses_on_face(current_obj: MeshObject, face_bb, bvh_cache_for_intersection: dict, placed_objects: [MeshObject], wall_obj: MeshObject):
        """
    ...
    def construct(used_floor_area: float, interior_objects: [MeshObject], materials: [Material], amount_of_extrusions: int = 0, fac_from_square_room: float = 0.3, corridor_width: float = 0.9, wall_height: float = 2.5, amount_of_floor_cuts: int = 2, only_use_big_edges: bool = True, create_ceiling: bool = True, assign_material_to_ceiling: bool = False, placement_tries_per_face: int = 3, amount_of_objects_per_sq_meter: float = 3.0):
        ...
        floor_obj, wall_obj, ceiling_obj = RandomRoomConstructor.construct_random_room(used_floor_area, amount_of_extrusions, fac_from_square_room, corridor_width, wall_height, amount_of_floor_cuts, only_use_big_edges, create_ceiling)
                ...
                if face_size < step_size:
                    ...
                    if current_accumulated_face_size > step_size:
                        for _ in range(placement_tries_per_face):
                            found_spot = RandomRoomConstructor.sample_new_object_poses_on_face(current_obj, face_bb, bvh_cache_for_intersection, placed_objects, wall_obj)
                            if found_spot:
