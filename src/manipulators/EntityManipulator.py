    ...
    def run(self):
        ...
        for key in params_conf.data.keys():
            ...
            for entity in entities:
                ...
                # used so we don't modify original key when having more than one object
                key_ = key
                ...
                if hasattr(entity, key_) and not demanded_custom_property:
                    # set the value
                    setattr(entity, key_, result)
                # if key had a cf_ prefix - treat it as a custom function.
                elif demanded_custom_function:
                    self._apply_function(entity, key_, result)
