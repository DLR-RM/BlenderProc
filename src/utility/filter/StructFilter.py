...
class StructFilter:
    @staticmethod
    def _check_its_one(structs: [Struct]) -> Struct:
        """ Checks if the given list only contains one element and returns it.

        :param structs: The list of elements to check.
        :return: The one element of the list.
        """
        if len(structs) != 1:
            raise Exception("More than one element with the given conditions has been found.")
        return structs[0]
    ...
    def by_attr(structs: [Struct], attr_name: str, value: Any, regex: bool = False) -> [Struct]:
        ...
        """
        return filter(lambda struct: StructFilter._check_equality(struct.get_attr(attr_name), value, regex), structs)
    ...
    def by_cp(structs: [Struct], cp_name: str, value: Any, regex: bool = False) -> [Struct]:
        ...
        """
        return filter(lambda struct: struct.has_cp(cp_name) and StructFilter._check_equality(struct.get_cp(cp_name), value, regex), structs)
